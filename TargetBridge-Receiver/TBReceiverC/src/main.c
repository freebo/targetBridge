...
static void tb_receiver_post_mouse_button(CGEventType type, CGMouseButton button, int click_count) {
    ...
    if (!event) return;
    CGEventSetIntegerValueField(event, kCGMouseEventClickState, click_count > 0 ? click_count : 1);
    CGEventPost(kCGHIDEventTap, event);
    CFRelease(event);
}
...
static void tb_receiver_send_input_button_event(struct app *a,
    ...
    a->input_events_sent += 1;
    if (tb_should_log_input_event(a->input_events_sent)) {
        tb_receiver_input_log("[input][receiver->sender] send #%llu kind=%s dx=0 dy=0 sx=0 sy=0 key=0 mode=%s",
                              (unsigned long long)a->input_events_sent,
                              kind ? kind : "?",
                              a->input_control_mode);
    }
...
static CGEventRef tb_receiver_input_tap_callback(CGEventTapProxy proxy,
    ...
    case kCGEventLeftMouseDown:
        tb_receiver_send_input_button_event(a, "leftDown",
                                            (int)CGEventGetIntegerValueField(event, kCGMouseEventClickState));
        should_consume = a->input_tap_consumes_events;
        break;
    case kCGEventLeftMouseUp:
        tb_receiver_send_input_button_event(a, "leftUp",
                                            (int)CGEventGetIntegerValueField(event, kCGMouseEventClickState));
        should_consume = a->input_tap_consumes_events;
        break;
    case kCGEventRightMouseDown:
        tb_receiver_send_input_button_event(a, "rightDown",
                                            (int)CGEventGetIntegerValueField(event, kCGMouseEventClickState));
        should_consume = a->input_tap_consumes_events;
        break;
    case kCGEventRightMouseUp:
        tb_receiver_send_input_button_event(a, "rightUp",
                                            (int)CGEventGetIntegerValueField(event, kCGMouseEventClickState));
        should_consume = a->input_tap_consumes_events;
        break;
    case kCGEventOtherMouseDown:
        tb_receiver_send_input_button_event(a, "otherDown",
                                            (int)CGEventGetIntegerValueField(event, kCGMouseEventClickState));
        should_consume = a->input_tap_consumes_events;
        break;
    case kCGEventOtherMouseUp:
        tb_receiver_send_input_button_event(a, "otherUp",
                                            (int)CGEventGetIntegerValueField(event, kCGMouseEventClickState));
        should_consume = a->input_tap_consumes_events;
        break;
